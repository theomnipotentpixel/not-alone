...
const g = p => {
	...
	p.quit = function() {
		if (host) {
			fetch("https://ns.jrtech.me:8080/flatline/" + peer.id)
				.then(res => res.text())
	...
	let heartbeatfunc = () => {
		fetch("https://ns.jrtech.me:8080/heartbeat/" + peer.id + "/" + host + "/" + VERSION + "/" + PATCH)
			.then(res => res.text())
	}
...
const m = p => {
	...
	function getRooms() {
		...
		fetch("https://ns.jrtech.me:8080/list/" + query)
			.then(res => res.text())
			.then(data => {
				...
				if (rooms.lobbies.length) for (let i = 0; i < rooms.lobbies.length; i++) {
					...
					btn.mouseClicked(() => {
						...
						fetch("https://ns.jrtech.me:8080/get/" + connID)
							.then(res => res.text())
							.then(data => {
								if (data == "oops")
